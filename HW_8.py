...
form = np.cov(zp, ks, ddof=0)

print('Задание 1.\n'
      f'Ковариация, посчитанная с помощью элементарных действий: {covar(zp,ks): .2f}\n'
      f'Ковариация, посчитанная с помощью функции: {form}\n')
...
print(f'Коэффициент корреляции, посчитанный вручную r = {r: .5f}\n')
r1 = np.cov(zp, ks, ddof=1) / (sigma(zp, offset=False) * sigma(ks, offset=False))
print(f'Коэффициент корреляции r = {r1}\n')
print(f'Коэффициент корреляции, посчитаннsq с помощью функции: r = {np.corrcoef(zp,ks)}\n')
...
print(f'Задание 2.\n'
      f'Среднее выборочное: {np.mean(arr): .2f},\n'
...
print(f'Табличное значение t-критерия для 95%-го доверительного '
      f'интервала данной выборки: {t_from_table(0.95, len(arr)): .3f}')
